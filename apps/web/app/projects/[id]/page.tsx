...
export default function ProjectDetailsPage() {
  ...
  const handleDeleteProject = async () => {
    if (!window.confirm(`Are you sure you want to permanently delete "${project?.name || 'this project'}" and all recorded videos, audio, and data?`)) {
      return;
    }
    setDeleting(true);
    setError(null);
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            <button
              ...
              disabled={deleting}
              className="px-2.5 py-1 rounded-lg text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 border border-slate-800 hover:border-rose-500/30 transition flex items-center gap-1.5 font-medium disabled:opacity-50"
          ...
          <div className={`mb-6 p-5 rounded-2xl backdrop-blur-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-2xl border ${
            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                ...
                disabled={retrying}
                className={`px-5 py-2.5 rounded-xl text-white font-semibold text-xs transition shadow-lg flex items-center gap-2 disabled:opacity-50 ${
                  project.status === 'COMPLETED'
              ...
              <button
                onClick={handleDeleteProject}
                disabled={deleting}
                className="px-4 py-2.5 rounded-xl text-rose-300 hover:text-white bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 font-semibold text-xs transition flex items-center gap-1.5 disabled:opacity-50"
                title="Permanently delete project and all data"
              >
